...
foundSpecifications.forEach { specFile ->
	...
	val basePackage = "org.ivanov.${packageName}"

	//генерация дто, интерфейсов и клиентов по спеке
...
val generatedJars = foundSpecifications.map { specFile ->
	...
	val generateSrcDir = outDirProvider.map { File(it).resolve("src/main/java") }
	 ...
	 println("[DEBUG] generateSrcDir for $name will be: ${generateSrcDir.get()}")
	...
	//компиляция ранее сгенерированных классов по спецификации
	...
	//упаковка скомпилированных классов отдельный jar архив
